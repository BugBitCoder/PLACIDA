...
function renderMessage(text, sender) {
  ...
  if (sender === 'bot') {
    if (typeof window.botMessageCount !== 'undefined') {
      window.botMessageCount++;
      if (typeof window.checkRealPersonBanner === 'function') {
        window.checkRealPersonBanner();
      }
    }
  }
}
...
let crisisMsgCount = 0;
const CRISIS_KEYWORDS = ['harm', 'hurt', 'end it', 'give up', 'kill', 'suicide', 'helpless'];
...
function sendMessage() {
  ...
  if (CRISIS_KEYWORDS.some(k => lowerText.includes(k))) {
    crisisMsgCount++;
    if (crisisMsgCount >= 3 && !sessionStorage.getItem('placida_crisis_modal_shown')) {
      const modal = document.getElementById('crisisModal');
      if (modal) modal.classList.add('visible');
      sessionStorage.setItem('placida_crisis_modal_shown', 'true');
    }
  }
