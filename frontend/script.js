...
/* Load user name from Supabase then render greeting */
function renderGreetingWithAuth() {
  ...
  if (window.supabase) {
    window.supabase.auth.getUser().then(({ data }) => {
      ...
      renderGreeting(name);
    }).catch(() => renderGreeting());
  } else {
    // Supabase not ready yet — listen for ready event
    ...
    renderGreeting(); // Show from localStorage or 'Friend' while loading
  }
}
